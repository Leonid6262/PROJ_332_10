...
*/
void CDMAcontroller::StartTxTransfer(unsigned char ch, unsigned int SrcAddr, unsigned int length) 
{
  DmaChannels[ch]->CSrcAddr = static_cast<unsigned int>((long long)SrcAddr);
  DmaChannels[ch]->CControl &= ~TransferSize_MASK;  
  DmaChannels[ch]->CControl |= (length & TransferSize_MASK);
  DmaChannels[ch]->CConfig |= GPDMA_DMACCxConfig_E;
}

void CDMAcontroller::StartRxTransfer(unsigned char ch, unsigned int DestAddr, unsigned int length) 
{
  DmaChannels[ch]->CDestAddr = static_cast<unsigned int>((long long)DestAddr);
  DmaChannels[ch]->CControl &= ~TransferSize_MASK;  
  DmaChannels[ch]->CControl |= (length & TransferSize_MASK);
  DmaChannels[ch]->CConfig |= GPDMA_DMACCxConfig_E;
}
...
void CDMAcontroller::init_M2P2M_Channel(const SChannelConfig *cfg) 
{
  ...
  LPC_GPDMACH_TypeDef* ch = DmaChannels[cfg->channelNum];
  
  // В любом случае сбрасывается Interrupt status
  LPC_GPDMA->IntTCClear = (1UL << cfg->channelNum);
  LPC_GPDMA->IntErrClr  = (1UL << cfg->channelNum);
  ...
  case ETransferType::TYPE_M2P:
    ...
    ch->CDestAddr = static_cast<unsigned int>((long long)(LUTPerAddr[cfg->Conn]));
    ...
    ch->CConfig = GPDMA_DMACCxConfig_TransferType(static_cast<unsigned int>(cfg->transferType)) | 
      GPDMA_DMACCxConfig_DestPeripheral(cfg->Conn);
  ...
  case ETransferType::TYPE_P2M:
    ...
    ch->CSrcAddr = static_cast<unsigned int>((long long)(LUTPerAddr[cfg->Conn]));
    ...
    ch->CConfig = GPDMA_DMACCxConfig_TransferType(static_cast<unsigned int>(cfg->transferType)) | 
      GPDMA_DMACCxConfig_SrcPeripheral(cfg->Conn);
